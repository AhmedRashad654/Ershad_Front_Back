...
const getApprovedFreelancers = async (req, res) => {
  try {
    ...
    console.log(req.query.englishLevel);
    if (
      page < 1 ||
      limit < 1 ||
    ...
    if (
      req.query.graduationYear &&
      req.query.graduationYear !== "null" &&
      req.query.graduationYear !== "undefined" &&
...
const getPendingFreelancers = async (req, res) => {
  try {
    ...
    if (req.query.willingToRelocate)
      filter.willingToRelocate = req.query.willingToRelocate;
    if (req.query.canWorkRemotely)
      filter.canWorkRemotely = req.query.canWorkRemotely;
...
const exportTableToCsv = async (req, res) => {
  try {
    ...
    const downloadLink = `${req.protocol}://${req.get(
      "host"
    )}/api/work/download/freelancers.csv`;
